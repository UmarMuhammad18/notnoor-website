...
function observeReveals() {
  document.querySelectorAll('.reveal:not(.on)').forEach(r => revObs.observe(r));
}
...
function initMobileMenu() {
  ...
  if (ham) ham.onclick = () => mobile.classList.add('open');
  if (close) close.onclick = () => mobile.classList.remove('open');
  window.closeMM = () => mobile.classList.remove('open');
}
...
function openProductDetail(id) {
  ...
  const waLink = `https://wa.me/2348020965505?text=${encodeURIComponent(p.waMsg || 'Hi I want to order ' + p.name)}`;
  document.getElementById('productDetailOrder').href = waLink;
...
function renderPublicTestimonials() {
  ...
  if (!testis.length) {
    grid.innerHTML = '<div class="no-products">No reviews yet. Be the first to leave a review!</div>';
    return;
  }
...
function renderLookbook() {
  ...
  grid.innerHTML = items.map((item, i) => {
    ...
    return `
      <div class="lb-item reveal d${(i % 4) + 1}">
        <div class="lb-vis">
          ${bgHTML}
          <div class="lb-shine"></div>
          <div class="lb-cap"><span class="lb-cap-text">${item.title}</span></div>
        </div>
      </div>
    `;
  }).join('');
...
function renderSiteContent() {
  ...
  if (btn1) {
    btn1.textContent = cfg.heroBtn1Text;
    if (cfg.heroBtn1Link.startsWith('#')) btn1.href = cfg.heroBtn1Link;
    else btn1.href = cfg.heroBtn1Link;
  }
  const btn2 = document.getElementById('heroBtn2');
  if (btn2) {
    btn2.textContent = cfg.heroBtn2Text;
    btn2.href = cfg.heroBtn2Link;
  }
  const btn3 = document.getElementById('heroBtn3');
  if (btn3) {
    btn3.textContent = cfg.heroBtn3Text;
    btn3.href = cfg.heroBtn3Link;
  }
  const marquee = document.getElementById('marqueeTrack');
  if (marquee) {
    marquee.innerHTML = (cfg.marqueeItems || []).map(item => `<span class="marquee-item">${item} <span>✦</span></span>`).join('');
  }
...
function loadAnnouncement() {
  ...
  if (toggle) toggle.checked = ann.active;
  const text = document.getElementById('annText');
  if (text) text.value = ann.text || '';
  updateAnnBar(ann);
}
function saveAnnouncement() {
  ...
  const ann = {
    ...
    color: getAnnouncement().color || '#c9a84c'
  };
  save(KEYS.announcement, ann);
  updateAnnBar(ann);
  const msg = document.getElementById('annSaveMsg');
  if (msg) {
...
function setAnnColor(c) {
  ...
  document.querySelectorAll('#annColorPicker > div').forEach(d => {
    d.style.borderColor = d.dataset.color === c ? 'var(--white)' : 'transparent';
  });
  updateAnnBar(ann);
}
...
function submitContactForm() {
  ...
  if (!name || !msg) {
    toast('Please fill in name and message');
    return;
  }
  ...
  if (document.getElementById('cf-msg')) document.getElementById('cf-msg').value = '';
...
// Override getProducts to inject base prices
const originalGetProducts = getProducts;
window.getProducts = function() {
  let prods = originalGetProducts();
  return prods.map(p => {
    if (!p.basePrice) {
...
// ========== ADMIN DASHBOARD (only on admin.html) ==========
...
function renderAdminProducts() {
  ...
  if (!grid) return;
  if (!prods.length) {
    grid.innerHTML = '<div style="grid-column:1/-1;text-align:center;padding:60px;color:rgba(250,248,245,.3);font-size:.85rem;">No products found. <button onclick="openAddProduct()" style="color:var(--gold);background:none;border:none;font-size:.85rem;">Add one +</button></div>';
    return;
  }
  grid.innerHTML = prods.map(p => {
...
function openAddProduct() {
  ...
  ['pName', 'pTag', 'pPrice', 'pDesc', 'pWAMsg', 'pDetails'].forEach(id => {
  ...
  const firstColor = document.querySelector('.color-chip[data-val="g1"]');
...
function saveProduct() {
  ...
  const hasImg = imgSrc && imgSrc !== '' && !imgSrc.endsWith('n0tn0or_full.html');
  const data = {
    name,
...
function toggleSizeChip(el) {
  ...
  _selectedSizes = Array.from(document.querySelectorAll('.size-chip.selected')).map(c => c.dataset.val);
}
...
function handleLookbookUpload(input) {
  ...
  const reader = new FileReader();
  reader.onload = e => {
    const prev = document.getElementById('lbImgPreview');
    prev.src = e.target.result;
    prev.style.display = 'block';
  };
  reader.readAsDataURL(file);
...
function openEditLookbookItem(id) {
  const item = getLookbook().find(x => x.id === id);
  if (!item) return;
...
function saveLookbookItem() {
  ...
  const hasImg = imgSrc && document.getElementById('lbImgPreview').style.display !== 'none';
...
function selectLbColor(el, val) {
  _selectedLbColor = val;
  document.querySelectorAll('#lbColorSwatches .color-chip').forEach(c => c.classList.remove('selected'));
  el.classList.add('selected');
}
...
function renderOrders() {
  ...
  if (!tbody) return;
  if (!orders.length) {
    tbody.innerHTML = '<tr><td colspan="7" style="text-align:center;padding:40px;color:rgba(250,248,245,.3);">No orders yet.</td></tr>';
    return;
  }
...
function openAddOrder() {
  ...
  ['oName', 'oCountry', 'oProd', 'oContact', 'oNotes'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.value = '';
  });
...
function saveOrder() {
  ...
  const data = {
    name,
    country: document.getElementById('oCountry').value.trim(),
    product: document.getElementById('oProd').value.trim(),
...
function renderTestimonialsAdmin() {
  ...
  if (!grid) return;
  if (!testis.length) {
    grid.innerHTML = '<div style="grid-column:1/-1;text-align:center;padding:60px;color:rgba(250,248,245,.3);">No reviews yet.</div>';
    return;
  }
...
function saveTestimonial() {
  ...
  const data = {
    name,
    loc: document.getElementById('tLoc').value.trim(),
    text,
    stars: parseInt(document.getElementById('tStars').value)
  };
  if (editId) {
    const idx = testis.findIndex(t => t.id == editId);
    if (idx > -1) testis[idx] = { ...testis[idx], ...data };
    addActivity(`Updated review: ${name}`);
  } else {
    data.id = Date.now();
    testis.push(data);
    addActivity(`Added review: ${name}`);
  }
...
function renderActivity() {
  ...
  if (!feed) return;
  if (!acts.length) {
    feed.innerHTML = '<p style="font-size:.8rem;color:rgba(250,248,245,.3);padding:12px 0;">No activity yet.</p>';
    return;
  }
  feed.innerHTML = acts.slice(0, 8).map(a => `
    <div class="activity-item"><div class="act-dot"></div><div><div class="act-text">${a.msg}</div><div class="act-time">${a.time}</div></div></div>
  `).join('');
}
...
function changePassword() {
  ...
  if (nw.length < 6) {
    msg.style.display = 'block';
    msg.style.color = '#e05050';
    msg.textContent = 'New password must be 6+ characters.';
    return;
  }
  if (nw !== cf) {
    msg.style.display = 'block';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (page === 'home') {
    ...
    // Other pages: sizing, delivery, order, contact
    applySettings();
    updateAnnBar(getAnnouncement());
  }
});
